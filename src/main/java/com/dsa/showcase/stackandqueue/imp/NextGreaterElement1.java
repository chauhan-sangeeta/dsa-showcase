...
public class NextGreaterElement1 {
    public static void main(String[] args) {
        ...
        // Optimised version
        int[] resultOptimised = nge.nextGreaterElementOptimised(nums1, nums2);
        for (int num : resultOptimised) {
            System.out.print(num + " ");
        }
    }
    ...
    public int[] nextGreaterElementOptimised(int[] nums1, int[] nums2) {
        ...
        for(int num:nums2){
            //descending order monotonic stack
            while(!st.isEmpty() && num>st.peek()){
                map.put(st.pop(),num);
            }
            st.push(num);
        }
        //st.empty()) map.put(st.pop(), -1); optional handle by getOrDefault
        int [] ans = new int[nums1.length];
        for(int i=0;i<nums1.length;i++){

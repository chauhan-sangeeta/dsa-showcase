package com.dsa.showcase.year2025.array.binarysearch.answerspace;
...
class MinimumNumberOfDaysToMakeMBouquets {
    ...
    public int countBouquet(int[] bloomDay, int day, int k) {
        int bouquet = 0;
        int count = 0;
        for (int d : bloomDay) {
            // If the flower blooms on or before the given day
            // and is adjacent to the previous bloomed flower
            if (d <= day) {
                count++;
            ...
            } else {// If the flower does not bloom on or before the given day
                count = 0;
            }
        }
        return bouquet;
